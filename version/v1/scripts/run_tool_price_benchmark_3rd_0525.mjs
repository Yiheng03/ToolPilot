...
const secondRoundDetailPath = path.join(resultsDir, "2nd_0525_detail.json")
...
const SECOND_ROUND_FALLBACK = {
  MAPE: 0.4774,
  MdAPE: null,
  within_30pct_rate: 0.5455,
  interval_hit_rate: 0.5606,
  mean_signed_percentage_error: 0.3002,
  overestimate_rate: 0.6667,
...
function buildSpecParts(item, formula, overrides = {}) {
  ...
  const drillDiameter = /3D钻头|5D钻头|深孔钻/.test(category)
    ? text(item.question).match(/(?:钻|直径|^|[^\d])(\d+(?:\.\d+)?)\s*mm(?:直径|的|3D|钻头)?/i)
    : null
  if (drillDiameter) {
...
const SPECIAL_BUCKETS = [
  ["H7球刀/铰刀", /H7|铰刀|球刀 螺旋铰刀/i],
  ["3D/5D钻头", /3D钻头|5D钻头|深孔钻/i],
  ["DLC单刃小径刀", /DLC涂层单刃|单刃|3\.175柄/i],
  ["台阶钻", /台阶钻/i],
  ["微型车刀/镗刀", /微型车刀|镗刀|MIR/i],
  ["加长铣刀", /加长|100L|120|150/i],
  ["T型刀/微小径深沟刀", /T型刀|微小径|深沟/i],
]

function specialBucket(row) {
  const value = `${row.category ?? ""} ${row.rawSpec ?? ""} ${row.question ?? ""}`
  return SPECIAL_BUCKETS.find(([, pattern]) => pattern.test(value))?.[0] ?? null
}

function bucketMetrics(rows) {
  const byBucket = {}
  for (const [label] of SPECIAL_BUCKETS) byBucket[label] = []
  for (const row of rows.filter((item) => item.eval_kind === "price" && !item.skipped_reason)) {
    const bucket = specialBucket(row)
    if (bucket) byBucket[bucket].push(row)
  }
  return Object.fromEntries(Object.entries(byBucket).map(([key, value]) => [key, slimGroup(value)]))
}

function loadSecondRoundDetail() {
  if (!fs.existsSync(secondRoundDetailPath)) return { overallMetrics: SECOND_ROUND_FALLBACK, details: [] }
  return JSON.parse(fs.readFileSync(secondRoundDetailPath, "utf8"))
}

function buildSummary({ runAt, market, overall, ranking, groupMetrics, generatedAnchors, details, secondRound }) {
  const topErrors = details
    .filter((row) => row.eval_kind === "price" && !row.skipped_reason)
    .sort((a, b) => b.percentage_error - a.percentage_error)
    .slice(0, 20)
  const second = secondRound.overallMetrics ?? SECOND_ROUND_FALLBACK
  const secondBuckets = bucketMetrics(secondRound.details ?? [])
  const thirdBuckets = bucketMetrics(details)
  const improved = overall.MAPE < second.MAPE && overall.mean_signed_percentage_error < second.mean_signed_percentage_error
  const conclusion = improved
    ? "第三轮相对第二轮有改善，分类子公式降低了整体 MAPE 和系统性高估。"
    : "第三轮相对第二轮改善不足，仍需要继续校准拖累品类。"
  return [
    ...
    "| --- | ---: | ---: |",
    `| MAPE | ${percent(second.MAPE)} | ${percent(overall.MAPE)} |`,
    `| MdAPE | ${percent(second.MdAPE)} | ${percent(overall.MdAPE)} |`,
    `| within_30pct_rate | ${percent(second.within_30pct_rate)} | ${percent(overall.within_30pct_rate)} |`,
    `| interval_hit_rate | ${percent(second.interval_hit_rate)} | ${percent(overall.interval_hit_rate)} |`,
    `| mean_signed_percentage_error | ${percent(second.mean_signed_percentage_error)} | ${percent(overall.mean_signed_percentage_error)} |`,
    `| overestimate_rate | ${percent(second.overestimate_rate)} | ${percent(overall.overestimate_rate)} |`,
    `| price_evaluated_questions | ${second.price_evaluated_questions} | ${overall.price_evaluated_questions} |`,
    `| ranking_evaluated_questions | ${second.ranking_evaluated_questions} | ${overall.ranking_evaluated_questions} |`,
    `| skipped_questions | ${second.skipped_questions} | ${overall.skipped_questions} |`,
    "",
    "## 总体价格指标",
    "",
    metricTable(overall),
    ...
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: |",
    ...SPECIAL_BUCKETS.map(([label]) => {
      const before = secondBuckets[label]
      const after = thirdBuckets[label]
      return `| ${label} | ${before?.count ?? 0} | ${percent(before?.MAPE)} | ${percent(before?.within_30pct_rate)} | ${after?.count ?? 0} | ${percent(after?.MAPE)} | ${percent(after?.within_30pct_rate)} |`
    }),
    "",
    "## 分 level 指标",
    "",
...
async function main() {
  ...
  const secondRound = loadSecondRoundDetail()
  ...
  fs.writeFileSync(summaryPath, buildSummary({ runAt, market, overall, ranking: rankingCore, groupMetrics, generatedAnchors, details, secondRound }), "utf8")
  fs.writeFileSync(
    errorsPath,
    [
